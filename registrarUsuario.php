...
<html class="wide wow-animation" lang="en">
  ...
  <body>
        ...
        $logUsu = Permitido($_POST['txtLogUsu']);
        $nomUsu = Permitido($_POST['txtNomUsu']);
        $carUsu = Permitido($_POST['cboCargo']);
        $passUsu =Permitido($_POST['txtPassUsu']);
        
        if (trim($logUsu)=="" || trim($nomUsu=="") || trim($passUsu)=="") {
            $contador=$contador+1;
        }
        if (is_numeric($nomUsu)){
            $contador=$contador=1;
        }
        if ($carUsu=="0") {
            $contador=$contador+1;
        ...
        if ($contador==0) {
                  $usu = new usuario(null,$logUsu,$nomUsu,$passUsu,$carUsu);
                 $a->registrarUsuario($usu);
                ...
                <div class="rd-navbar-main-element">
                  <div class="rd-navbar-nav-wrap">
                    <ul class="rd-navbar-nav">
                      ...
                      <li class="rd-nav-item active"><a class="rd-nav-link" href="registrarUsuario.php">Registrar Usuario</a>
                      </li>
          ...
          <div class="row row-40 justify-content-center">
            <div class="col-lg-6 col-12">
              <div class="offset-top-45 offset-lg-right-45">
                <h3 class="wow fadeInLeft text-capitalize" data-wow-delay=".3s">Registrar Usuario</h3>
				
				<!--Inicio partes para rellenar -->
				<form method="post" action="registrarUsuario.php" name="registrarUsuario" id="registrarUsuario">
				
                <p class="font-weight-bold text-gray-dark wow fadeInUp" data-wow-delay=".4s">
				<label for="txtLogUsu" name="txtLogUsu">Login de Usuario</label>
                                <input required="" type="text" name="txtLogUsu" id="txtLogUsu" class="form-control"> 
				</p>
				
                <p class="font-weight-bold text-gray-dark wow fadeInUp" data-wow-delay=".4s">
				<label for="txtNomUsu" name="txtNomUsu">Nombre del Usuario</label>
                                <input required="" type="text" id="txtNomUsu" name="txtNomUsu" class="form-control">
				</p>
				
				
                <p class="font-weight-bold text-gray-dark wow fadeInUp" data-wow-delay=".4s"> 
						<label for="txtPassUsu" name="txtPassUsu">Contraseña</label>
                                                <input type="password" required="" id="txtPassUsu" name="txtPassUsu" class="form-control">
				</p>
				<p class="font-weight-bold text-gray-dark wow fadeInUp" data-wow-delay=".4s"> 
						<label for="cboCargo" name="cboCargo">Cargo: </label>
				...
				<p class="font-weight-bold text-gray-dark wow fadeInUp" data-wow-delay=".4s"> 
						<button type="submit" name="btnRegUsuario" class="form-control">Registrar Usuario</button>
				</p>
              </div>
            </div>
            <div class="col-lg-6 col-sm-10 col-12">
              <div class="block-decorate-img wow fadeInLeft" data-wow-delay=".2s">
                  <table  border="1" id="divTablaUsuario">
                      <tr>
                      <th>Nombre</th>
                      <th>Login</th>
                      <th>Cargo</th>
            ...
            for($i=0; $i < count($lista); $i++){
                $usuLis = $lista[$i];
                $idUsuLis = $usuLis->getIdUsuario();
                $logUsuLis = $usuLis->getLogUsuario();
                $nomUsuLis = $usuLis->getNombreUsuario();
                $carUsuLis = $usuLis->getCargoUsuario();
                
                
                echo "<tr>";
                echo "<td>$nomUsuLis</td>";
                echo "<td>$logUsuLis</td>";
                echo "<td>$carUsuLis</td>";
                echo "<td><a href='registrarUsuario.php?ideli=$idUsuLis'>AQUI</a></td>";
